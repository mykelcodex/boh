  ...
  <main>
    <div class="border-b border-line bg-green-50">
      <div class="wrap max-w-[820px] py-[72px]">
        ...
        <p class="lead">
          Each programme runs inside juvenile centres and correctional facilities, and continues after release. Together they cover skills, schooling, work, leadership, healing, reform, and the community a young person returns to.
        </p>
      </div>
    </div>
        ...
        <div v-if="layout === 'cards'" class="grid gap-5 md:grid-cols-2">
          <article v-for="p in programmes" :key="p.n" class="card gap-3.5">
            <div class="flex items-center gap-3.5">
              ...
              <span class="font-mono text-xs text-muted">{{ p.n }}</span>
            </div>
            <h2 class="m-0 font-sans text-[clamp(19px,4vw,22px)] font-semibold text-ink">{{ p.title }}</h2>
            <p class="m-0 text-base leading-[1.65]">{{ p.body }}</p>
          </article>
        </div>

        <div v-else class="overflow-hidden rounded-md border border-line bg-white shadow-e1">
          <div v-for="p in programmes" :key="p.n" class="border-b border-line last:border-0">
            <button
            ...
            >
              <span class="font-mono text-xs text-muted">{{ p.n }}</span>
              <span class="flex-1 font-sans text-[19px] font-semibold text-ink">{{ p.title }}</span>
              <AppIcon
                name="chevronDown"
                :size="20"
            ...
            <p v-if="open === p.n" class="m-0 max-w-[760px] px-6 pb-6 pl-6 text-base leading-[1.65] sm:pl-14">{{ p.body }}</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="section bg-green-700 text-white">
      <div class="wrap max-w-[900px] text-center">
        ...
        <h2 class="h-section mb-4 !text-white">Back one programme, or bring your organisation in</h2>
        <p class="lead mx-auto mb-9 max-w-[620px] !text-green-200">
          Fund a trade workshop, sponsor exam fees for a cohort, mentor a graduate through their first year of work, or open a placement at your company.
        </p>
        <div class="flex flex-wrap justify-center gap-3">
          <button class="btn-gold" @click="give.open()">Fund a programme</button>
          <NuxtLink to="/contact" class="btn-outline-light no-underline">Become a partner or mentor</NuxtLink>
        </div>
      </div>
    </section>
  </main>
...
useSeoMeta({
  ...
  description: 'Trade and vocational training, formal schooling access, job placement, leadership, reintegration, advocacy, and community engagement.'
})
</script>
